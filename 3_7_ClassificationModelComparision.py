# -*- coding: utf-8 -*-
"""
Created on Thu Jul 17 16:11:13 2025
...
# İlk veri kümesi görselleştirme
...
# Sınıflandırıcılarla karar sınırı görselleştirmesi
fig = plt.figure(figsize=(6, 9))
i = 1
for ds_cnt, ds in enumerate(datasets):
    X, y = ds
    ...
    for name, clf in zip(names, classifiers):
        ...
        if ds_cnt == 0:
            ax.set_title(name)
        ax.text(X[:, 0].max() + 0.25, X[:, 1].min() - 0.45, f"{score:.2f}")
        i += 1
